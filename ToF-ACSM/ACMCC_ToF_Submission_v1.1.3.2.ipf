...
//	Button LoadConcButt, title="\\f01 1.Load Tofware conc. File", pos={64,238},fSize=18,size={280,35},font="Arial", fcolor=(52224,34816,0), proc=LoadACSMConcFile
	Button LoadACSMButt, title="\\f01 2.Load Native Files", pos={108,253},fSize=16,size={185,27},font="Arial", fcolor=(52224,34816,0), proc=LoadACSMDataFiles
	Button GetACSMButt, title="\\f01 3.Get ACSM data", pos={100,282},fSize=16,size={200,27},font="Arial", fcolor=(52224,34816,0), proc=GetACSM_proc
	Button CheckLODButton, title="\\f01 4.Check LOD values", pos={10,314},fSize=14,size={150,27},font="Arial", fcolor=(52224,34816,0), proc=CheckLODPanel
...
//	CheckBox UseMiddlebrook_CB, title="Use Composition-Dependent CE", pos={175,281}, fsize=14,value=ApplyMiddlebrook, proc=CE_Warning_proc
	Button GetDryerdata_butt, title="5.Dryer", pos={181,314}, size={60,25},font="Arial", fcolor=(52224,34816,0),proc=LoadDryerData//, disable=2
	Button CEpanel_Butt, title="\\f01 6.CDCE panel", pos={276,314},fSize=14,size={110,25},font="Arial", fcolor=(52224,34816,0), proc=CE_ButtonProc
...
//	NVAR/Z ApplyMiddlebrook=root:ACMCC_Export:ApplyMiddlebrook	
//	print ApplyMiddlebrook
//	if(ApplyMiddlebrook==0)
		 duplicate/O CEW CE
//	Elseif(ApplyMiddlebrook==1)
...
Function LoadDryerData(ctrlName) : ButtonControl
	...
	if (strlen(outputPaths) == 0)
		...
		//added for RH to be used in CDCE	
...
Function AutoFlagPanel(ctrlName) : ButtonControl
	...
	SetVariable InletPmax_Var, title="max InletP", pos={220,10}, value=InletPmax, size={100,15}
 	...
 	CheckBox InletPvar_CB, title="InletP variation", pos={10,43}, fsize=14, variable=AutoFlag_InletPvar
 	SetVariable InletPvar_Var, title="abs threshold (torr)", pos={150,43}, value=InletPvar, size={150,15}
 	
 	CheckBox Airbeam_CB, title="Airbeam signal", pos={10,80}, fsize=14, variable=AutoFlag_AB
 	SetVariable AB_Warning_var, title="High limit AB", pos={150,80}, value=AB_high, size={140,15}
 	SetVariable AB_limit_var, title="Low limit AB", pos={295,80}, value=AB_low, size={140,15}

	CheckBox VapT_CB, title="Vap. temperature", pos={10,115}, fsize=14, variable=AutoFlag_VapT
 	SetVariable VapTmin_var, title="VapT min", pos={150,115}, value=VapTmin, size={100,15}
 	SetVariable VapTmax_var, title="VapT max", pos={270,115}, value=VapTmax, size={100,15}
 	
 	CheckBox ConcLOD_CB, title="Concentration LOD", pos={10,150}, fsize=14, variable=AutoFlag_ConcLOD
 	
 	CheckBox RH_CB, title="Dried RH", pos={10,210}, fsize=14, variable=AutoFlag_RH
 	SetVariable RH_var, title="RH max", pos={210 ,210}, value=RHmax, size={90,15}
 	
 	CheckBox Concvar_CB, title="Concentration variation", pos={10,180}, fsize=14, variable=AutoFlag_Concvar
 	SetVariable Concvar_var, title="abs threshold", pos={240,180}, value=Concvar, size={120,15}
 	
 	CheckBox PMdiff_CB, title="Mass Closure", pos={10,240}, fsize=14, variable=AutoFlag_PMdiff,value=0,disable=2
...
Function CheckDoublonAndNaN()
	...
	SetDataFolder root:ToF_ACSM
	wave DateNative, Mx
	Make/O/N=(numpnts(DateNative)) d_date
	
	d_date[0]=999
	
	variable i
	for(i=1;i<numpnts(DateNative);i+=1)
		d_date[i]=DateNative[i]-DateNative[i-1]
	endfor

	d_date[]=(numtype(DateNative[p])==2) ? 0 : d_date[p]
